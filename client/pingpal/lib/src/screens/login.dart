...
class _LoginPageState extends State<LoginPage> {
  ...
  void initState() {
    ...
    const String webClientId = '801373348761-9br5ei64qv6reu56ia5s96guu3im9mim.apps.googleusercontent.com';
    ...
    print('GoogleSignIn initialized with serverClientId: ${_googleSignIn.serverClientId}');
  }
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Text('Server Client ID: ${_googleSignIn.serverClientId ?? 'Not set'}'),
  ...
  Future<void> _tryFirebaseAuthFlow() async {
    try {
      ...
      if (idToken != null) {
        ...
        _showErrorDialog('Authentication Error', 'Failed to obtain Google ID token');
      }
    } catch (e) {
    ...
    try {
      ...
      if (account != null) {
        ...
        if (auth.idToken != null) {
          Map<String, dynamic> tokenInfo = TokenInspector.decodeToken(auth.idToken!);
  ...
  Future<void> _testDirectApiCall() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('API Health check: ${response.statusCode} - ${response.body}')),
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (account != null) {
        ...
        if (idToken == null) {
          ...
          _showErrorDialog('Authentication Error', 'Failed to obtain ID token from Google');
      ...
      if (mounted) {
        _showErrorDialog('Sign-In Failed', 'Error during Google Sign-In: $error');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<bool> _authenticateWithServer(String idToken) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Server authentication failed with status code ${response.statusCode}');
        try {
      ...
      _showErrorDialog('Connection Error', 'Failed to connect to authentication server: $e');
